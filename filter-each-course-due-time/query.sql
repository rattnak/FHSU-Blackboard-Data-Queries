...
INNER JOIN cdm_lms.term term ON term.id = c.term_id
...
GROUP BY
    c.id, c.name, c.design_mode, term.start_date, term.name, gb.name, gb.due_time, ih.hierarchy_name_seq
ORDER BY term.start_date, c.name, gb.due_time ASC;
