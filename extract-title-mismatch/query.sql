...
WHERE 
    -- Normalize extracted term formats to match `lt.name`
    COALESCE(
        REGEXP_SUBSTR(lc.name, '[A-Z]20[0-9]{2}'),  -- Format: F2025 (only years starting with 20)
        CONCAT(RIGHT(REGEXP_SUBSTR(lc.name, '20[0-9]{2}[A-Z]'), 1), 
               LEFT(REGEXP_SUBSTR(lc.name, '20[0-9]{2}[A-Z]'), 4))  -- Format: 2025F → F2025
