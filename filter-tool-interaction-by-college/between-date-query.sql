-- CTE: Collect tool usage details per person in each course
WITH tool_usage AS (
    SELECT
        cta.course_id,
        cta.tool_id,
    ...
    GROUP BY cta.course_id, cta.tool_id, t.name, cta.person_course_id, ih.hierarchy_name_seq
),
-- CTE: Aggregate tool usage per course per tool
...
-- CTE: Aggregate tool interaction details per course
tool_agg_per_course AS (
    SELECT
        course_id,
        LISTAGG(tool_name || ': ' || total_interactions, ', ') WITHIN GROUP (ORDER BY tool_name) AS tool_interaction_details,
...
SELECT
    ...
    tag.tool_interaction_details,
    tag.total_tool_interaction_count,
    TO_CHAR(pta.last_accessed_time, 'YYYY-MM-DD HH24:MI:SS') AS last_accessed_time
