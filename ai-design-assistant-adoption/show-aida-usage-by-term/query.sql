-- PURPOSE: SHOW AIDA USAGE BY TERM
with ai_summary as (
select
...
select term, count(distinct course_id) as course_count, count(course_item_id) as item_count from ai_summary where ai_status = 'Y' group by 1 order by 3 desc;